...
func listen(address string) error {
	var err error
	listenFD, err = syscall.Socket(syscall.AF_INET, syscall.SOCK_STREAM, 0)//参数1 面向网络的 参数2 面向连接的套接字
	if err != nil {
		log.Error(err)
		return err
	}
	err = syscall.SetsockoptInt(listenFD, syscall.SOL_SOCKET, syscall.SO_REUSEADDR, 1) //解决结束进程后再次启动提示 Address already in use问题
	if err != nil {
		log.Error(err)
		return err
	}
