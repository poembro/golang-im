var app=new Vue({
    ...
    data: {
        ...
        server:"ws://192.168.84.168:7923/ws",//server:getWsBaseUrl()+"/chat_server",
        socket:null,
        messageContent:"", //请输入内容 
        msgList:[], // 消息列表  
        shop:{ //客服配置信息
            shop_id : "13200000000",
            device_id:"web_13200000000",
            shop_name : "客服丽丽",
        ...
        user:{ //用户配置信息
            ...
            is_online: true,
            last_message: ["{\"user_id\":405342717686579210, \"shop_id\":13200000000, \"type\":\"text\", \"msg\":\"3333\", \"room_id\":\"31bf19de027be5029716b460be4c16bd\", \"dateline\":1651132706, \"id\":\"1651132706180189000\"}"],
            nickname: "-",
            platform: "-",
            pushurl: "-",
            referer: "-",
    ...
    methods: {
        ...
        OnMessage(msg) {
            ...
            if (msg.user_id == _this.shop.shop_id) 
            {
                dst.is_shop = true
        ...
        talkTo(user) {//接手客户
            ...
            this.chatTitle = "正在与" + user.user_id + "用户聊天...";
            
            // 清空上一个人的聊天记录
            this.msgList = []

            //获取历史消息列表
            this.getMesssagesByUserId(user.user_id);
            for(var i=0;i<this.onlineUsers.length;i++){
                if(this.onlineUsers[i].user_id==user.user_id){
        ...
        listIpblack(){
            ...
            _this.sendAjax("/open/listIpblack","post",{shop_id:shop_id}, function(dst){
                var result = dst.list
        ...
        addIpblack(ip){ // ip添加至黑名单
            ...
            _this.sendAjax("/open/addIpblack","post",{shop_id:shop_id, ip:ip}, function(dst){
        ...
        delIpblack(ip){ // ip从黑名单删除
            let _this=this
            let shop_id = _this.shop.shop_id 
            _this.sendAjax("/open/delIpblack","post",{shop_id:shop_id, ip:ip}, function(dst){
        ...
        getOnlineUser(){//获取在线用户信息
            let _this=this
            let shop_id = _this.shop.shop_id
            _this.sendAjax("/open/finduserlist","post",{shop_id:shop_id, typ:"online"}, function(dst){
                if (!dst) {
        ...
        getOfflineUser(page){  //获取离线用户
            ...
            let _this=this
            let shop_id = _this.shop.shop_id
            _this.sendAjax("/open/finduserlist?page="+page, "post",{shop_id:shop_id, typ:"offline"}, function(dst){
                if (!dst) {
        ...
        sendMsg (){
            let _this = this;
            let dst = {
                user_id: _this.shop.shop_id,
                shop_id: _this.user.user_id,
                room_id:_this.user.room_id,
                type:'text',
                msg :_this.messageContent
            } 
            this.sendAjax("/open/push", "post", dst, function(result){
                console.log(result)
            });
        ...
        sendAjax(url,method,params,callback){
            let _this=this;
            url = "http://192.168.84.168:8090" + url
            $.ajax({
                type: method,
                url: url,
                data:params,
