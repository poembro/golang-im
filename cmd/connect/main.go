...
func main() {
    ...
    // 初始化Rpc Client
    rpc.InitLogicIntClient(config.Global.GrpcSchema, config.Global.EtcdAddr)
    ...
    go func() {
        ...
        _, err := rpc.LogicIntClient.ServerStop(context.TODO(), &pb.ServerStopReq{ConnAddr: config.Connect.LocalAddr})
        if err != nil {
            panic(err)
        }
