(function(win) {
    ...
    Client.prototype.createConnect = function(max, delay) {
        ...
        function connect() {
            ...
            function auth() {
                ...
                var token = '{"mid":' + self.options.mid + ', "room_id":"' + self.options.room_id + '", "platform":"' + self.options.platform + '", "accepts":[' + self.options.accepts + ']}'
            ...
            function subscribedRoom() {
                //协议格式对应 /api/comet/grpc/protocol
                var token = 'live://800020210817001'
                //var token = '{"mid":' + self.options.mid + ', "room_id":"' + self.options.room_id + '", "platform":"' + self.options.platform + '", "accepts":[' + self.options.accepts + ']}'
                var headerBuf = new ArrayBuffer(rawHeaderLen); 
                var headerView = new DataView(headerBuf, 0);
                var bodyBuf = textEncoder.encode(token); //接收一个String类型的参数返回一个Unit8Array 1个字节
                ...
                headerView.setInt16(headerOffset, rawHeaderLen); //写入从内存的第4个字节序开始  值为16
                headerView.setInt16(verOffset, 1); //版本号为1
                headerView.setInt32(opOffset, 14);  //写入从内存的8第个字节序开始，值为7 标识auth
                headerView.setInt32(seqOffset, 1); //从内存的12个字节序开始· 值为1   序列号（服务端返回和客户端发送一一对应）
                ws.send(mergeArrayBuffer(headerBuf, bodyBuf));

                appendMsg("send: subscribedRoom token: " + token);
            }
    ...
    Client.prototype.sendMsg = function(msg) {
        var self = this;  
        sendToMsg(msg)
        function sendToMsg(msg) {
            //协议格式对应 /api/comet/grpc/protocol
            var token = '{"type":"text","room_id":"xxx","msg":"' + msg + '"}'
            //var token = '{"mid":' + self.options.mid + ', "room_id":"' + self.options.room_id + '", "platform":"' + self.options.platform + '", "accepts":[' + self.options.accepts + ']}'
            var headerBuf = new ArrayBuffer(rawHeaderLen); 
            var headerView = new DataView(headerBuf, 0);
            var bodyBuf = new TextEncoder().encode(token); //接收一个String类型的参数返回一个Unit8Array 1个字节
            headerView.setInt32(packetOffset, rawHeaderLen + bodyBuf.byteLength); //包长度  写入从内存的第0个字节序开始  值为16 + token长度
            headerView.setInt16(headerOffset, rawHeaderLen); //写入从内存的第4个字节序开始  值为16
            headerView.setInt16(verOffset, 1); //版本号为1
            headerView.setInt32(opOffset, 4);  //写入从内存的8第个字节序开始，值为4 标识发消息
            headerView.setInt32(seqOffset, new Date().getTime()); //从内存的12个字节序开始· 值为1   序列号（服务端返回和客户端发送一一对应）
