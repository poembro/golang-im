...
func (h *Router) apiFindUserInfo(w http.ResponseWriter, r *http.Request) {
	var (
		userId string
		uInfo  model.User
	)
	...
	if userId == "" {
		OutJson(w, OutData{Code: -1, Success: false, Msg: "参数user_id不能为空"})
		return
	}
	...
	OutJson(w, OutData{Code: 200, Success: true, Result: uInfo})
}

// apiFindUserList 查看所有与自己聊天的用户
func (h *Router) apiFindUserList(w http.ResponseWriter, r *http.Request) {
	var (
		typ    string
		shopId string
	...
	if shopId == "" {
		OutJson(w, OutData{Code: -1, Success: false, Msg: "参数shop_id不能为空"})
		return
	}
	ids := make([]int64, 0)
	// 查询在线人数
