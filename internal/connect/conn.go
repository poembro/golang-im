...
func (c *Conn) MessageACK(p *protocol.Proto) {
    ...
    _, err := rpc.LogicIntClient.MessageACK(grpclib.ContextWithRequstId(context.TODO(), int64(p.Seq)),
        &pb.MessageACKReq{
            UserId:      c.UserId,
            DeviceId:    c.DeviceId,
...
func (c *Conn) Sync(p *protocol.Proto) {
    resp, err := rpc.LogicIntClient.Sync(grpclib.ContextWithRequstId(context.TODO(), int64(p.Seq)), // 333 对应 time.Now().UnixNano()
        &pb.SyncReq{
            UserId:   c.UserId,
            DeviceId: c.DeviceId,
            Seq:      2,
...
func (c *Conn) SignIn(p *protocol.Proto) {
    resp, err := rpc.LogicIntClient.ConnSignIn(grpclib.ContextWithRequstId(context.TODO(), int64(p.Seq)),
        &pb.ConnSignInReq{
            Body:       p.Body,
...
func (c *Conn) OpSendMsg(p *protocol.Proto) {
    ...
    _, err := rpc.LogicIntClient.SendMessage(ctx,
        &pb.PushMsgReq{
            Message: buf,
        })
    if err != nil {
